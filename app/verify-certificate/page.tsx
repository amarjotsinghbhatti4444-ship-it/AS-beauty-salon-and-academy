...
export default function VerifyCertificatePage() {
  ...
  const handleSearch = async (e: React.FormEvent) => {
    ...
    setCertificateData(error ? null : data);
    setHasSearched(true);
  };
    ...
    <div className="flex flex-col py-12 md:py-24 bg-gray-50 min-h-[90vh]">
      <div className="container mx-auto px-4 lg:px-8">
        <div className="mx-auto max-w-2xl text-center mb-12">
          ...
          <motion.h1 initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="font-serif text-3xl font-bold tracking-tight text-gray-900 md:text-4xl">
            Verify Student Certificate
          </motion.h1>
          <motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="mt-4 text-gray-600">
        ...
        <div className="mx-auto max-w-xl">
          <form onSubmit={handleSearch} className="mb-8">
            <div className="relative flex items-center">
              ...
              />
              <button type="submit" className="absolute right-2 top-2 bottom-2 rounded-xl bg-black px-6 text-sm font-medium text-white transition-colors hover:bg-rose-500 flex items-center">
              ...
              >
                {certificateData ? (
                  <>
                    ...
                    <div className="space-y-6 relative z-10">
                      <div>
                        ...
                        <div className="text-xl font-serif font-bold text-gray-900">{certificateData.student_name}</div>
                      </div>
                      <div className="grid grid-cols-2 gap-6 pt-4 border-t border-gray-100">
                        <div>
                          <div className="text-xs font-semibold text-gray-400 tracking-widest uppercase mb-1">Course Name</div>
                          <div className="text-sm font-medium text-gray-800">{certificateData.course_name}</div>
                        </div>
                        <div>
                          <div className="text-xs font-semibold text-gray-400 tracking-widest uppercase mb-1">Certificate ID</div>
                          <div className="text-sm font-mono font-medium text-gray-600">{certificateData.certificate_id}</div>
                        </div>
                        <div>
                          <div className="text-xs font-semibold text-gray-400 tracking-widest uppercase mb-1">Issue Date</div>
                          <div className="text-sm font-medium text-gray-800">{new Date(certificateData.issue_date).toLocaleDateString()}</div>
                        </div>
                        <div>
                          <div className="text-xs font-semibold text-gray-400 tracking-widest uppercase mb-1">Status</div>
                          <div className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-green-100 text-green-800">Active</div>
                        </div>
                      </div>
                      {certificateData.certificate_image && (
                        <div className="pt-6 border-t border-gray-100 mt-6">
                          ...
                          <img src={certificateData.certificate_image} alt={`Certificate for ${certificateData.student_name}`} className="w-full h-auto rounded-xl border border-gray-200 shadow-sm object-contain max-h-[600px] bg-gray-50" />
                        </div>
                      )}
                    </div>
                  </>
                  ...
                  <div className="text-center py-8">
                    ...
                    <p className="text-gray-500 text-sm">We could not find a verified certificate associated with {mobileNumber}. Please ensure you entered the correct number.</p>
                  </div>
                )}
              </motion.div>
            )}
