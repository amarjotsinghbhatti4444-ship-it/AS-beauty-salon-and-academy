...
export default function AdminDashboardPage() {
  ...
  const fetchCertificates = async () => {
    ...
    if (!error && data) {
      setCertificates(data.map((c: any) => ({
        ...
        certificateImage: c.certificate_image,
      })));
    }
  };
  ...
  const handleAdd = async (e: React.FormEvent) => {
    ...
    if (!error) {
      ...
      setMobile(''); setStudentName(''); setCourseName('');
      setIssueDate(''); setCertificateImage('');
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 max-w-6xl">
        <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4 mb-8">
          <div>
            ...
            <h1 className="font-serif text-3xl sm:text-4xl font-bold text-gray-800">Certificate Management</h1>
          </div>
          <div className="flex items-center gap-3">
            <button onClick={handleLogout} className="flex items-center gap-2 px-4 py-2.5 rounded-lg border border-gray-200 bg-white text-sm font-medium text-gray-700 hover:bg-gray-50 transition-colors">
              <LogOut className="w-4 h-4" /> Logout
            </button>
            <button onClick={() => setIsModalOpen(true)} className="flex items-center gap-2 px-5 py-2.5 rounded-lg bg-[#5A3A31] text-white text-sm font-medium hover:bg-[#4a2f27] transition-colors">
        ...
        <div className="bg-white rounded-xl border border-gray-200 shadow-sm mb-6 flex flex-col sm:flex-row items-center justify-between p-2 pl-4">
          <div className="flex items-center flex-1 w-full text-gray-400">
            ...
            <input type="text" placeholder="Search by name, phone, or cert ID..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full bg-transparent border-none outline-none text-gray-700 text-sm py-2 placeholder-gray-400" />
          </div>
                      ...
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-2">
                          {cert.certificateImage && (
                            <button onClick={() => setViewImage(cert.certificateImage!)} className="flex items-center justify-center w-8 h-8 rounded-full bg-blue-50 text-blue-500 hover:bg-blue-100 transition-colors">
                              <ImageIcon className="w-3.5 h-3.5" />
                            </button>
                          )}
                          <button className="flex items-center justify-center w-8 h-8 rounded-full bg-gray-100 text-gray-500 hover:bg-gray-200 transition-colors">
                            <Edit2 className="w-3.5 h-3.5" />
                          </button>
                          <button onClick={() => handleDelete(cert.id)} className="flex items-center justify-center w-8 h-8 rounded-full bg-rose-50 text-rose-500 hover:bg-rose-100 transition-colors">
      ...
      {viewImage && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
          <div className="relative w-full max-w-4xl mx-auto flex flex-col items-center">
            <button onClick={() => setViewImage(null)} className="absolute -top-12 right-0 p-2 text-white/70 hover:text-white bg-black/50 hover:bg-black/80 rounded-full transition-colors">
              <X className="w-6 h-6" />
            </button>
